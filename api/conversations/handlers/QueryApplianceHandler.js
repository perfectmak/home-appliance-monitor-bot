...
function isConversationGrounded(conversationState) {
  ...
  return !!(entities.appliance && entities.context);
}
...
function requestForEntity(sendMessage, entityName) {
  switch(entityName) {
    case Entity.Appliance:
      return sendMessage('Which appliance are you interested in?');
    case Entity.Context:
...
/**
 * Query for the status of the appliance
 *
 * @param ApplianceService
 * @param sendMessage
 * @param conversationState
 */
function queryAppliance(ApplianceService, sendMessage, conversationState) {
  const entities = conversationState.entities;
  ...
  const applianceCriteria = { name: entities.appliance, context: entities.context };
    ...
    return ApplianceService.fetchForUser(userId, applianceCriteria)
      .then((appliances) => {
        ...
        const CommandBuilder = CreateCommand({ value: entities.state, pin: pin })
          .read();

        const command = (appliance.type === 'switch') ?
          CommandBuilder.digital().build() :
          CommandBuilder.analog().build();

        return sendMessage(`Okay, I'm check for the state of the ${entities.appliance} now.`)
          .then(() => {
            return ApplianceService.sendCommandToDevice(deviceId, command);
          })
          .then((responseMsg) => {
            const state = (appliance.type === 'switch') ? responseMsg : `at level ${responseMsg}`;
            sendMessage(`The ${entities.appliance} is ${state}`)
          })
          .catch(sendMessage);
      });
...
const QueryApplianceHandler = {
  ...
  process({ MessengerService, ConversationService, ApplianceService }, user, message, conversationState, nlpResponse) {
    ...
    const newConversationState = (conversationState) ?
        ...
        {
          userId: user.id,
          intent: nlpResponse.intent, // QueryAppliance
          entities: {}
        };
      ...
      ApplianceService.fetchAllForUser(user.id)
            ...
            case 0:
              postUrl(`Sorry, I can\'t find any appliance with the name '${entities.appliance}'.` +
                ...
                sendMessage('Or perhaps you mean\'t one of these?',
                  lodash.uniq(appliances.map(lodash.property('name'))));
              ...
              if(entities.context) {
                ...
                if(correctAppliance) {
                  ...
                  sendMessage('Or perhaps you mean\'t in one of these places?',
                    lodash.uniq(matchedAppliances.map(lodash.property('context'))));
        ...
        .then(savedConversationState => {
          if(savedConversationState.grounded) {
            queryAppliance(ApplianceService, sendMessage, savedConversationState)
...
module.exports = QueryApplianceHandler;
