 ...
 * @param conversationState
 */
function switchAppliance(ApplianceService, sendMessage, conversationState) {
  const entities = conversationState.entities;
  ...
  const applianceCriteria = { name: entities.appliance, context: entities.context };
    ...
    return ApplianceService.fetchForUser(userId, applianceCriteria)
      .then((appliances) => {
        ...
        const command = CreateCommand({ value: entities.state.toLowerCase(), pin: pin })
          .write().digital().build();
...
module.exports = SwitchApplianceHandler;
